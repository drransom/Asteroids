(function(){
  ...
  Ellipse.prototype.createVector = function (theta) {
    var xcord = this.xradius * Math.sin(theta);
    var ycord = this.yradius * Math.cos(theta);
    return new SAT.Vector(xcord, ycord).rotate(this.offset);
  };

  Asteroids.Util.generateRandomEllipse = function (radius) {
    var xradius = ((Math.random() * 0.5) + 0.33) * radius;
    var yradius = Math.sqrt(Math.pow(radius, 2) - Math.pow(xradius, 2));
